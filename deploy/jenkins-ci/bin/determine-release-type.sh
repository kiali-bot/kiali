...
# It is possible to specify a different date using
...
#     release shoudl be built (specifically, the first
#     snapshot of the sprint.
# - "snapshot.0": if it's determined that a snapshot
#     release shoudl be built (specifically, the second
#     snapshot of the sprint.
#
# The reference date (base date) can be set in the
# environment variable BASE_DATE. By default, it is the
# last day of Sprint #14.
#
# Both NOW_DATE and BASE_DATE should be given in seconds
# since EPOCH. It is assumed that this script is run weekly
# since the base date. Running at different timespans
# won't guarantee a good result.

# The date when the Jenkins Pipelile was first used to
# make a release. This is the last day of Sprint #14.
...
# This value will be used to determine the type o the release
